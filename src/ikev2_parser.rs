...
pub fn parse_ikev2_payload_kex<'a>(
    ...
    do_parse!(
        ...
        dh: be_u16
            ...
            >> data: take!(length - 4)
            >> (IkeV2PayloadContent::KE(KeyExchangePayload {
...
pub fn parse_ikev2_payload_ident_init<'a>(
    ...
    do_parse!(
        ...
        id_type: be_u8
            ...
            >> error_if!(length < 4, ErrorKind::Custom(128))
            >> data: take!(length - 4)
            >> (IkeV2PayloadContent::IDi(IdentificationPayload {
...
pub fn parse_ikev2_payload_ident_resp<'a>(
    ...
    do_parse!(
        ...
        id_type: be_u8
            ...
            >> error_if!(length < 4, ErrorKind::Custom(128))
            >> data: take!(length - 4)
            >> (IkeV2PayloadContent::IDr(IdentificationPayload {
                id_type: IdentificationType(id_type),
                reserved1: reserved1,
                reserved2: reserved2,
                ident_data: data,
...
pub fn parse_ikev2_payload_certificate<'a>(
    ...
    do_parse!(
        ...
        encoding: be_u8
            ...
            >> data: take!(length - 1)
            >> (IkeV2PayloadContent::Certificate(CertificatePayload {
...
pub fn parse_ikev2_payload_certificate_request<'a>(
    ...
    do_parse!(
        ...
        encoding: be_u8
            >> error_if!(length < 1, ErrorKind::Custom(128))
            >> data: take!(length - 1)
            >> (IkeV2PayloadContent::CertificateRequest(CertificateRequestPayload {
                cert_encoding: CertificateEncoding(encoding),
                ca_data: data,
            }))
    )
...
pub fn parse_ikev2_payload_authentication<'a>(
    ...
    do_parse!(
        ...
        method: be_u8
            >> error_if!(length < 4, ErrorKind::Custom(128))
            >> data: take!(length - 4)
            >> (IkeV2PayloadContent::Authentication(AuthenticationPayload {
...
pub fn parse_ikev2_payload_nonce<'a>(
    ...
    do_parse!(
        i,
        data: take!(length) >> (IkeV2PayloadContent::Nonce(NoncePayload { nonce_data: data }))
    )
}
...
pub fn parse_ikev2_payload_notify<'a>(
    ...
    do_parse!(
        ...
        proto_id: be_u8
            ...
            >> notif_data:
                cond!(
                    length > 8 + spi_sz as u16,
                    take!(length - (8 + spi_sz as u16))
                )
...
pub fn parse_ikev2_payload_ts<'a>(
    ...
    do_parse!(
        ...
        num_ts: be_u8
            >> reserved: take!(3)
            >> error_if!(length < 4, ErrorKind::Custom(128))
            >> ts: flat_map!(take!(length - 4), many1!(complete!(parse_ikev2_ts)))
...
mod tests {
    ...
    static IKEV2_INIT_REQ: &'static [u8] = &[
        ,
    ];
    ...
    static IKEV2_PAYLOAD_SA: &'static [u8] = &[
        0x22, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x24, 0x01, 0x01, 0x00, 0x03, 0x03, 0x00, 0x00,
        0x0c, 0x01, 0x00, 0x00, 0x14, 0x80, 0x0e, 0x00, 0x80, 0x03, 0x00, 0x00, 0x08, 0x02, 0x00,
        0x00, 0x05, 0x00, 0x00, 0x00, 0x08, 0x04, 0x00, 0x00, 0x1e,
    ];
